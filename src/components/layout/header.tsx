...
import { Languages, Menu, Moon, Search, Shield, Sun, X } from "lucide-react";
...
export function Header() {
	...
	return <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/70">
		<div className="container flex h-16 items-center justify-between gap-4">
			<Link href="/" className="flex shrink-0 items-center gap-2" onClick={() => setOpen(false)}><Shield className="h-6 w-6 text-primary" aria-hidden="true" /><span className="text-lg font-bold">指纹浏览器资源库</span></Link>
			...
			<div className="flex items-center gap-1">
				<Link href="/browsers/" className={cn(buttonVariants({ variant: "ghost", size: "icon" }), "hidden md:inline-flex")} aria-label="搜索资源"><Search className="h-5 w-5" /></Link>

...
import { allLocaleParams, hreflangFor, isLocale, localeMeta, localePath, pageMeta, resourceFor, t, type Locale, type LocalizedKind } from "@/lib/i18n";
...
function Detail({ locale, kind, item }: { locale: Locale; kind: LocalizedKind; item: BrowserResource | ToolResource | TechnologyResource | GuideResource | ComparisonResource }) {
	...
	if (kind === "browsers" && "price" in item) return <Shell locale={locale} path={path}><article className="mx-auto max-w-5xl"><header className="mb-10 flex flex-wrap items-center gap-4"><ResourceIcon name={item.name} src={item.icon} website={item.website} size={64} /><div><h1 className="text-3xl font-semibold tracking-tight sm:text-4xl">{item.name}</h1><div className="mt-2 flex flex-wrap items-center gap-2"><StatusBadge status={item.status} /><Rating value={item.rating} /></div></div></header><p className="mb-10 max-w-3xl text-lg leading-8 text-muted-foreground">{item.longDescription}</p><div className="grid gap-5 md:grid-cols-3"><Card><CardHeader><CardTitle className="text-base">{labels.specs}</CardTitle></CardHeader><CardContent className="space-y-3 text-sm"><div className="flex justify-between gap-4"><span className="text-muted-foreground">Free</span><span>{item.free}</span></div><div className="flex justify-between gap-4"><span className="text-muted-foreground">Price</span><span className="font-mono">{item.price}</span></div><div className="flex justify-between gap-4"><span className="text-muted-foreground">Profiles</span><span>{item.profiles}</span></div><div className="flex justify-between gap-4"><span className="text-muted-foreground">Platform</span><span>{item.platforms.join(", ")}</span></div></CardContent></Card><Card className="md:col-span-2"><CardHeader><CardTitle className="text-base">{labels.features}</CardTitle></CardHeader><CardContent><ul className="grid gap-3 sm:grid-cols-2">{item.features.map(feature => <li key={feature} className="flex items-start gap-2 text-sm"><Check className="mt-0.5 h-4 w-4 shrink-0 text-success" />{feature}</li>)}</ul></CardContent></Card></div><section className="mt-10 grid gap-5 md:grid-cols-2"><Card><CardHeader><CardTitle className="text-base">{labels.bestFor}</CardTitle></CardHeader><CardContent><ul className="space-y-2 text-sm leading-7">{item.bestFor.map(value => <li key={value}>• {value}</li>)}</ul></CardContent></Card><Card><CardHeader><CardTitle className="text-base">{labels.limitations}</CardTitle></CardHeader><CardContent><ul className="space-y-2 text-sm leading-7">{item.cons.map(value => <li key={value}>• {value}</li>)}</ul></CardContent></Card></section><a href={`${item.website}${item.sponsored ? "?utm_source=browserhub&utm_medium=referral&utm_campaign=resource_hub" : ""}`} target="_blank" rel="noopener noreferrer" className={cn(buttonVariants(), "mt-10")}><ExternalLink className="mr-2 h-4 w-4" />{labels.official}</a><p className="mt-8 border-t pt-5 text-xs leading-6 text-muted-foreground">{labels.sourceNote} · {labels.updated} {item.updatedAt}</p></article></Shell>;
	if (kind === "tools" && "pricing" in item) return <Shell locale={locale} path={path}><article className="mx-auto max-w-4xl"><header className="mb-10 flex items-center gap-4"><ResourceIcon name={item.name} src={item.icon} website={item.website} size={60} /><div><h1 className="text-3xl font-semibold tracking-tight sm:text-4xl">{item.name}</h1><p className="mt-2 text-sm text-muted-foreground">{item.category} · {item.platforms.join(", ")}</p></div></header><p className="text-lg leading-8 text-muted-foreground">{item.longDescription}</p><h2 className="mt-10 text-xl font-semibold">{labels.features}</h2><ul className="mt-5 grid gap-3 sm:grid-cols-2">{item.features.map(feature => <li key={feature} className="rounded-lg border bg-card px-4 py-3 text-sm"><Check className="mr-2 inline h-4 w-4 text-success" />{feature}</li>)}</ul><h2 className="mt-10 text-xl font-semibold">{labels.howTo}</h2><ol className="mt-4 list-decimal space-y-3 pl-6 text-sm leading-7 text-muted-foreground"><li>Open the official resource and review its privacy notes.</li><li>Run the test in the browser environment you want to evaluate.</li><li>Compare results with independent tools and record the test conditions.</li></ol><a href={item.website} target="_blank" rel="noopener noreferrer" className={cn(buttonVariants(), "mt-10")}><ExternalLink className="mr-2 h-4 w-4" />{labels.official}</a></article></Shell>;
	if (kind === "fingerprint" && "doc" in item) return <Shell locale={locale} path={path}><article className="mx-auto max-w-4xl"><header className="mb-10"><p className="eyebrow text-primary">{item.category}</p><h1 className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">{item.name} · {labels.fingerprint}</h1><p className="mt-5 text-lg leading-8 text-muted-foreground">{item.description}</p></header><div className="grid gap-5 md:grid-cols-2"><Card><CardHeader><CardTitle className="text-base">What</CardTitle></CardHeader><CardContent><p className="text-sm leading-7 text-muted-foreground">{item.description} This signal is normally evaluated together with device, browser, network and account context.</p></CardContent></Card><Card><CardHeader><CardTitle className="text-base">{labels.detection}</CardTitle></CardHeader><CardContent><p className="text-sm leading-7 text-muted-foreground">Compare the signal in multiple browsers and networks, then cross-check with public detection tools.</p></CardContent></Card></div><h2 className="mt-10 text-xl font-semibold">{labels.mitigation}</h2><p className="mt-4 leading-8 text-muted-foreground">Keep browser, device, language, timezone and network settings consistent. Follow the target website terms and local laws.</p><a href={item.doc} target="_blank" rel="noopener noreferrer" className={cn(buttonVariants({ variant: "outline" }), "mt-8")}><ExternalLink className="mr-2 h-4 w-4" />{labels.official}</a></article></Shell>;
	if (kind === "guides" && "content" in item) return <Shell locale={locale} path={path}><article className="mx-auto max-w-3xl"><header className="mb-10"><Badge variant="secondary">{item.category}</Badge><h1 className="mt-4 text-3xl font-semibold tracking-tight sm:text-4xl">{item.title}</h1><p className="mt-5 text-lg leading-8 text-muted-foreground">{item.description}</p></header><Card><CardHeader><CardTitle>{item.title}</CardTitle></CardHeader><CardContent><p className="text-sm leading-8 text-muted-foreground">{item.content}</p></CardContent></Card><p className="mt-8 border-l-2 border-primary bg-primary/[0.04] px-4 py-3 text-sm leading-7 text-muted-foreground">{labels.sourceNote}</p></article></Shell>;
	if (kind === "compare" && "browsers" in item) { const a = browsers.find(browser => browser.slug === item.browsers[0]) ?? browsers[0]; const b = browsers.find(browser => browser.slug === item.browsers[1]) ?? browsers[1]; const rows: Array<[string, string | boolean, string | boolean]> = [["Free", a.free, b.free], ["Price", a.price, b.price], ["Profiles", a.profiles, b.profiles], ["Platform", a.platforms.join(", "), b.platforms.join(", ")], ["Engine", a.engine, b.engine], ["API", a.api, b.api], ["Automation", a.automation, b.automation], ["Proxy", a.proxy, b.proxy]]; return <Shell locale={locale} path={path}><article><header className="mb-10"><h1 className="flex flex-wrap items-center gap-4 text-3xl font-semibold tracking-tight sm:text-4xl"><span>{a.name}</span><span className="rounded-full border px-3 py-1 font-mono text-sm text-muted-foreground">vs</span><span>{b.name}</span></h1><p className="mt-5 max-w-3xl leading-7 text-muted-foreground">{item.description} {labels.compareIntro}</p></header><Card><CardHeader><CardTitle>{labels.comparisonTable}</CardTitle></CardHeader><CardContent className="overflow-x-auto p-0"><table className="w-full min-w-[560px] text-sm"><thead><tr className="border-y bg-muted/50 text-left"><th className="p-4">Field</th><th className="p-4">{a.name}</th><th className="p-4">{b.name}</th></tr></thead><tbody>{rows.map(([label, va, vb]) => <tr key={label} className="border-b"><td className="p-4 font-medium text-muted-foreground">{label}</td><td className="p-4">{typeof va === "boolean" ? va ? <Check className="h-4 w-4 text-success" /> : <Minus className="h-4 w-4 text-muted-foreground" /> : va}</td><td className="p-4">{typeof vb === "boolean" ? vb ? <Check className="h-4 w-4 text-success" /> : <Minus className="h-4 w-4 text-muted-foreground" /> : vb}</td></tr>)}</tbody></table></CardContent></Card></article></Shell>; }
	notFound();
}

...
export default function Home() {
	...
	return <div className="flex flex-col">
		...
		<section className="py-16 md:py-20"><div className="container"><div className="mb-10 flex flex-wrap items-end justify-between gap-4"><div><h2 className="text-3xl font-bold tracking-tight">特色推荐</h2><p className="mt-2 text-muted-foreground">Featured 资源与常用产品并列展示，信息以官方页面为准。</p></div><Link href="/browsers/" className={buttonVariants({ variant: "outline" })}>查看全部 <ArrowRight className="ml-2 h-4 w-4" /></Link></div><div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">{featuredBrowsers.map(browser => <Card key={browser.slug} className={browser.featured ? "border-primary shadow-md" : ""}><CardHeader><div className="flex items-start justify-between gap-3"><div className="flex items-center gap-3"><ResourceIcon name={browser.name} src={browser.icon} /><CardTitle className="text-xl">{browser.name}</CardTitle></div>{browser.featured && <Badge>Featured</Badge>}</div><CardDescription className="pt-2">{browser.description}</CardDescription></CardHeader><CardContent><dl className="space-y-2 text-sm"><div className="flex justify-between gap-3"><dt className="text-muted-foreground">状态</dt><dd className="text-right font-medium">{browser.status}</dd></div><div className="flex justify-between gap-3"><dt className="text-muted-foreground">免费额度</dt><dd className="text-right font-medium">{browser.free}</dd></div><div className="flex justify-between gap-3"><dt className="text-muted-foreground">起步价格</dt><dd className="text-right font-medium">{browser.price}</dd></div></dl><Link href={`/browsers/${browser.slug}/`} className={cn(buttonVariants({ variant: browser.featured ? "default" : "outline" }), "mt-4 w-full")}>查看详情 <ArrowRight className="ml-2 h-4 w-4" /></Link></CardContent></Card>)}</div></div></section>
